...
比較対象は **(c) で実際に指定したベース参照をそのまま使う**。
ローカルから切ったなら `<base>`、`origin/<base>` から切ったなら `origin/<base>`。
ローカルに存在しないブランチ名を書くと `unknown revision` で落ちる。

```bash
# (c) で `git switch -c <new-branch> <base>` とした場合
git log --oneline <base>..HEAD

# (c) で `git switch -c <new-branch> origin/<base>` とした場合
git log --oneline origin/<base>..HEAD
```

**出力が空であること**を確認する。空でなければベースを取り違えている。

その場合は **`-c` ではなく `-C` で切り直す**。同名のブランチが既に存在するため、
`-c` は `fatal: a branch named '<new-branch>' already exists` で失敗する。

```bash
git switch -C <new-branch> <base>   # (c) と同じベース参照を指定する
```

`-C` は「無ければ作る、あれば指し先を移す」であり、まだ何もコミットしていない
この時点では失うものがない。ルール 6 の要承認操作にも該当しない
（`git branch -D` を使う必要はない）。

ただし **既にこのブランチでコミットしてしまっている場合は `-C` を使わない**。
そのコミットが到達不能になる。その場合はルール 9「PR 作成前の内容確認」の
復旧手順に従う。
...
**この判断はユーザー確認必須。** 混入したコミットが本当に無関係なのか、
他の PR で既に扱われているのかは、こちらからは判断できない。
`AskUserQuestion` で下の選択肢を提示する（非対話時は推奨案 A を前提に進め、
最終応答で選択肢と採用理由を提示する）。

推奨は **A**。既存のコミットを一切壊さず、force push もルール 6 の
要承認操作も発生しないため。

**A. ベースから切り直して、必要なコミットだけ cherry-pick する（推奨）**

```bash
git switch <base>
git pull --rebase origin <base>
git switch -c <new-branch> <base>      # ルール 3(c)。ベースを明示する
git log --oneline <base>..HEAD          # ルール 3(d)。空であること
git cherry-pick <必要なコミット>         # 複数ある場合は古い順に並べる
git log --oneline <base>..HEAD          # 意図したコミットだけになったか再確認
```

元のブランチはそのまま残るので、取り違えたコミットの退避先を兼ねる。
`cherry-pick` は git が自動生成するコミットなのでルール 4 の例外にあたる
（`git-commit` スキルへの委譲は不要）。

**B. 説明文に明記して 1 本の PR のまま出す**

分離できない場合（混入コミットに依存していて cherry-pick が衝突する等）や、
どちらもすぐレビューしてほしい場合に選ぶ。含まれるコミットを本文に列挙し、
**両方がレビュー対象であることを冒頭で明示する**。

**選んではいけない対処**

現ブランチ上で `git rebase -i <base>` して混入コミットを drop するのは、
...
- [ ] **ブランチを切る場合、`git switch -c <new> <base>` とベースを明示したか？**
      （引数を省くと現在の HEAD から切られ、直前の作業ブランチのコミットを引き連れる）
- [ ] **派生直後に `git log --oneline <base>..HEAD` が空であることを確認したか？**
- [ ] **PR を作る前に `git log --oneline <base>..HEAD` を説明文と突き合わせたか？**
